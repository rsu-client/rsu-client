...
sub unix_main
{
	...
	# Set the cachedir location and enable TLS
	$rsu_data->javabin = $rsu_data->javabin." -Dhttps.protocols=TLSv1,TLSv1.1,TLSv1.2 -Duser.home=\"".$rsu_data->cachedir."\"";
...
sub windows_main
{
	...
	# Set the cachedir location and enable TLS
	$win32javabin = "$win32javabin -Dhttps.protocols=TLSv1,TLSv1.1,TLSv1.2 -XX:+AggressiveOpts -Duser.home=\"".$rsu_data->cachedir."\"";
...
sub checkcompabilitymode
{
	...
	if (($rsu_data->args =~ /--compabilitymode/ && $rsu_data->OS !~ /MSWin32/) || ($rsu_data->compabilitymode =~ /(1|true)/i && $rsu_data->OS !~ /MSWin32/))
	{
		...
		# Finish the launch code and enable TLS
		$launchline = "$launchline -Dhttps.protocols=TLSv1,TLSv1.1,TLSv1.2 -cp $params /share/img/$iconfolder && exit\"";
